...
export default function SwapContainer() {
  const [showFromTokenTooltip, setShowFromTokenTooltip] = useState(false);
  const [showToTokenTooltip, setShowToTokenTooltip] = useState(false);
  ...
  const handleFromTokenSelect = (token: any) => {
    setFromToken(token);
    setShowFromTokenTooltip(false);
  };

  const handleToTokenSelect = (token: any) => {
    setToToken(token);
    setShowToTokenTooltip(false);
  };
  ...
  // Calculate receive amount based on from token, to token, and input amount
  const calculateReceiveAmount = () => {
    if (!amount || parseFloat(amount) <= 0) return "0";

    // Simple conversion logic - in real app this would use actual exchange rates
    const inputAmount = parseFloat(amount);
    let receiveAmount = 0;

    if (fromToken.id === "icp" && toToken.id === "btc") {
      receiveAmount = inputAmount * 0.000001; // Example rate
    } else if (fromToken.id === "btc" && toToken.id === "icp") {
      receiveAmount = inputAmount * 1000000; // Example rate
    } else if (fromToken.id === "icp" && toToken.id === "eth") {
      receiveAmount = inputAmount * 0.001; // Example rate
    } else if (fromToken.id === "eth" && toToken.id === "icp") {
      receiveAmount = inputAmount * 1000; // Example rate
    } else {
      // Default 1:1 conversion for other tokens
      receiveAmount = inputAmount;
    }

    return receiveAmount.toLocaleString("en-US", {
  ...
  return (
    <div className="overflow-hidden relative w-full h-full flex flex-col rounded-lg">
      ...
      {/* Main content */}
      <div className="flex flex-col gap-[14px] items-center justify-center flex-1 px-4">
        {/* Title section */}
        <div className="flex flex-col items-center justify-center pb-15">
          <div className="text-[#545454] text-6xl text-center font-bold uppercase w-full">swapping</div>
          <div className="flex gap-[5px] items-center justify-center w-full">
            <div className="text-[#545454] text-6xl text-center font-bold uppercase">Any T</div>
            <div className="h-[48px] relative rounded-full w-[125.07px] border-[4.648px] border-primary border-solid"></div>
            <div className="text-[#545454] text-6xl text-center font-bold uppercase">ken</div>
          </div>
        </div>

        {/* Visual divider with "To" indicator */}
        <div className="flex flex-row items-center justify-center w-full max-w-md h-[50px] relative">
          {/* From Token selector */}
          <div className="flex gap-1 items-center justify-center w-full max-w-md">
            <div
              ...
              data-tooltip-id="from-token-selector-tooltip"
              onClick={() => setShowFromTokenTooltip(true)}
            >
              <img src="/arrow/caret-down.svg" alt="Select from token" className="w-5 h-5" />
              <img src={fromToken.icon} alt={fromToken.name} className="w-9 h-9" />
            </div>
          </div>
          ...
          <div className="h-[75.46px] w-full max-w-[528px] flex items-center justify-center relative">
            ...
            <div className="relative w-full h-full">
              <div
                className={`absolute left-0 top-1/2 w-full h-0.5 border-t border-dashed transform -translate-y-1/2 transition-colors duration-300 ease-in-out ${
                  fromToken.id !== toToken.id && amount.trim() !== "" && fromToken.id !== "icp" && toToken.id !== "icp"
                    ? "border-[#0059ff]"
                    : "border-gray-300"
                }`}
              />
              {/* Left arrow */}
              <div
                className={`absolute w-0 h-0 border-t-[4px] border-t-transparent border-b-[4px] border-b-transparent border-r-[6px] left-[-6px] top-1/2 transform -translate-y-1/2 transition-colors duration-300 ease-in-out ${
                  fromToken.id !== toToken.id && amount.trim() !== "" && fromToken.id !== "icp" && toToken.id !== "icp"
                    ? "border-r-[#0059ff]"
                    : "border-r-gray-300"
                }`}
              ...
              {/* Right arrow */}
              <div
                className={`absolute w-0 h-0 border-t-[4px] border-t-transparent border-b-[4px] border-b-transparent border-l-[6px] right-[-6px] top-1/2 transform -translate-y-1/2 transition-colors duration-300 ease-in-out ${
                  fromToken.id !== toToken.id && amount.trim() !== "" && fromToken.id !== "icp" && toToken.id !== "icp"
                    ? "border-l-[#0059ff]"
                    : "border-l-gray-300"
                }`}
            ...
            <div className="absolute bg-[#f4f4f6] rounded-[32.842px] w-8 h-8 top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 flex items-center justify-center">
              <img src="/swap/swap-icon.svg" alt="To" className="w-5 h-5" />
              <div
          ...
          <div className="flex gap-1 items-center justify-center w-full max-w-md">
            <div
              className="bg-white flex gap-1 items-center justify-start pl-1.5 pr-0.5 py-0.5 rounded-full border border-[#e0e0e0] cursor-pointer"
              data-tooltip-id="to-token-selector-tooltip"
              onClick={() => setShowToTokenTooltip(true)}
            >
              <img src="/arrow/caret-down.svg" alt="Select to token" className="w-5 h-5" />
              <img src={toToken.icon} alt={toToken.name} className="w-9 h-9" />
            </div>
          </div>
        </div>
        ...
        <input
          ...
          className="text-text-primary text-[44px] uppercase outline-none w-[180px] text-center"
        />

        {/* Receive amount display */}
        {amount.trim() !== "" && fromToken.id !== toToken.id && fromToken.id !== "icp" && toToken.id !== "icp" && (
          <div className="flex flex-col gap-[5px] items-center justify-center w-full max-w-xl">
            <span className="text-text-secondary text-[16px]">
              You'll receive [<span className="text-primary uppercase">{toToken.symbol}</span>]
            </span>
            {/* Amount display */}
            <span className="text-text-primary uppercase outline-none text-center text-[44px]">
        ...
        <div className="flex gap-2 items-center justify-center w-full max-w-xs">
          ...
          <button className="bg-gradient-to-b from-[#48b3ff] to-[#0059ff] flex items-center justify-center px-5 py-2 rounded-[10px] shadow-[0px_2px_4px_-1px_rgba(12,12,106,0.5),0px_0px_0px_1px_#4470ff] flex-1">
            <span className="font-semibold text-[16px] text-center text-white tracking-[-0.16px]">Swap now</span>
          </button>
        </div>
        ...
        {/* From Token selection tooltip */}
        <Tooltip
          id="from-token-selector-tooltip"
          isOpen={showFromTokenTooltip}
          place="left"
          openOnClick={true}
          border="1px solid #0059ff"
          ...
          offset={340}
          render={() => (
            <SelectTokenTooltip onTokenSelect={handleFromTokenSelect} onClose={() => setShowFromTokenTooltip(false)} />
          )}
        />

        {/* To Token selection tooltip */}
        <Tooltip
          id="to-token-selector-tooltip"
          isOpen={showToTokenTooltip}
          place="right"
          openOnClick={true}
          border="1px solid #0059ff"
          className="!bg-transparent !border-0 !shadow-none !p-0 !-mt-5"
          ...
          render={() => (
            <SelectTokenTooltip
              ...
              onClose={() => setShowToTokenTooltip(false)}
              arrowPosition="right"
            />
          )}
        />
